...
!- Read umask and e3u in parent grid file:
...
!- Read e2u and e3u in parent grid file:
  ...
  DO kmonth=1,12
    ...
    DO kday=1,31
      ...
      IF ( existfile ) THEN
        ...
        status = NF90_OPEN(TRIM(file_data_zgr),0,fidZGRIN);     call erreur(status,.TRUE.,"alethea hacks")
    !   status = NF90_INQ_VARID(fidZGRIN,"e3u_0",e3u_ID)
        status = NF90_INQ_VARID(fidZGRIN,"e3u",e3u_ID)
        if ( status .ne. 0 ) then
          write(*,*) 'No e3u found in grid_U file => assuming not in vvl mode (i.e. constant e3u)'
          ln_vvl = .false.
        else
    !     ALLOCATE( e3u_0(mlon,mlat,mdepthu,mtime)  )
          ALLOCATE( e3u(mlon,mlat,mdepthu,mtime)  )
          write(*,*) 'Barotropic component calculated from time-dependent e3u'
    !     status = NF90_GET_VAR(fidZGRIN,e3u_ID,e3u_0) ; call erreur(status,.TRUE.,"getvar_e3u")
          status = NF90_GET_VAR(fidZGRIN,e3u_ID,e3u) ; call erreur(status,.TRUE.,"getvar_e3u")
          ln_vvl = .true.
        endif
        status = NF90_CLOSE(fidZGRIN);                          call erreur(status,.TRUE.,"end alethea hacks")
