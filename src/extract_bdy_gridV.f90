...
DO kyear=nn_yeari,nn_yearf
  ...
  DO kmonth=1,12
   ...
   !    status = NF90_INQ_VARID(fidZGRIN,"e3v_0",e3v_ID)
        status = NF90_INQ_VARID(fidZGRIN,"e3v",e3v_ID)
        if ( status .ne. 0 ) then
          write(*,*) 'No e3v found in grid_V file => assuming not in vvl mode (i.e. constant e3v)'
          ln_vvl = .false.
        else
   !      ALLOCATE( e3v_0(mlon,mlat,mdepthv,mtime)  )
          ALLOCATE( e3v(mlon,mlat,mdepthv,mtime)  )
          write(*,*) 'Barotropic component calculated from time-dependent e3v'
   !      status = NF90_GET_VAR(fidZGRIN,e3v_ID,e3v_0) ; call erreur(status,.TRUE.,"getvar_e3v")
          status = NF90_GET_VAR(fidZGRIN,e3v_ID,e3v) ; call erreur(status,.TRUE.,"getvar_e3v")
          ln_vvl = .true.
        endif

        status = NF90_CLOSE(fidZGRIN)                                     ; call erreur(status,.TRUE.,"end of alethea hacks")     
