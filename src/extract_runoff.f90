...
DO kyear=nn_rrr_yeari,nn_rrr_yearf
  ...
  DO kmonth=1,12
   ...
   !    status = NF90_INQ_VARID(fidRNF,"time",time_ID)      ; call erreur(status,.TRUE.,"inq_time_ID")
        status = NF90_INQ_VARID(fidRNF,"sorunoff",runoff_ID)
        ...
        status = NF90_DEF_VAR(fidM,"time_counter",NF90_DOUBLE,(/dimID_time_counter/),time_counter_ID)
        call erreur(status,.TRUE.,"def_var_time_counter_ID")
        status = NF90_DEF_VAR(fidM,"runoff",NF90_FLOAT,(/dimID_x,dimID_y,dimID_time_counter/),runoff_ID)
        ...
        status = NF90_PUT_ATT(fidM,time_counter_ID,"units",TRIM(time_units))            ; call erreur(status,.TRUE.,"put_att_time_ID")
        status = NF90_PUT_ATT(fidM,time_counter_ID,"calendar",TRIM(calendar))           ; call erreur(status,.TRUE.,"put_att_time_ID")
        status = NF90_PUT_ATT(fidM,time_counter_ID,"title","Time")                      ; call erreur(status,.TRUE.,"put_att_time_ID")
        status = NF90_PUT_ATT(fidM,time_counter_ID,"long_name","Time axis")             ; call erreur(status,.TRUE.,"put_att_time_ID")
        status = NF90_PUT_ATT(fidM,time_counter_ID,"standard_name","time")              ; call erreur(status,.TRUE.,"put_att_time_ID")
        status = NF90_PUT_ATT(fidM,time_counter_ID,"axis","T")                          ; call erreur(status,.TRUE.,"put_att_time_ID")
